...
class PoseVelocityCommand(CommandTerm):
    ...
    def robot_velocity_w(self) -> torch.Tensor:
        """The current base velocity in the world frame. Shape is (num_envs, 6)."""
        return self.robot.data.root_lin_vel_w
    
    @property
    def robot_pos_w(self) -> torch.Tensor:
        """The current base position in the world frame. Shape is (num_envs, 3)."""
        return self.robot.data.root_pos_w
    
    @property
    def target_pos_w(self) -> torch.Tensor:
        """The current target position in the world frame. Shape is (num_envs, 3)."""
