...
struct HasPressedTime : std::false_type {};

template <typename T>
struct HasPressedTime<T, std::void_t<decltype(std::declval<const T&>().pressed_time)>> : std::true_type {};

inline float EstimatePressedTimeFallback(std::string_view key_name, bool pressed)
{
  using Clock = std::chrono::steady_clock;
  static std::unordered_map<std::string, Clock::time_point> pressed_since;
  const auto now = Clock::now();
  const std::string key = std::string(key_name);

  if (!pressed) {
    pressed_since.erase(key);
    return 0.0f;
  }

  auto [it, inserted] = pressed_since.emplace(key, now);
  if (inserted) {
    return 0.0f;
  }
  return std::chrono::duration<float>(now - it->second).count();
}

template <typename KeyT>
inline float ReadPressedTime(const KeyT& key, std::string_view key_name)
{
  if constexpr (HasPressedTime<KeyT>::value) {
    return static_cast<float>(key.pressed_time);
  } else {
    return EstimatePressedTimeFallback(key_name, key.pressed);
  }
}

template <typename KeyT>
inline KeyState MakeKeyState(const KeyT& key, std::string_view key_name)
{
  KeyState state;
  state.pressed = key.pressed;
  state.on_pressed = key.on_pressed;
  state.on_released = key.on_released;
  state.pressed_time = ReadPressedTime(key, key_name);
  return state;
}
...
inline KeyState GetKey(const UnitreeJoystick& joy, std::string_view name_sv) {
  ...
  using GetterFn = KeyState (*)(const UnitreeJoystick&, std::string_view);
  static const std::unordered_map<std::string, GetterFn> kMap = {
    {"back", [](const UnitreeJoystick& j, std::string_view n)->KeyState { return MakeKeyState(j.back, n); }},
    {"start",[](const UnitreeJoystick& j, std::string_view n)->KeyState { return MakeKeyState(j.start, n); }},
    {"ls",   [](const UnitreeJoystick& j, std::string_view n)->KeyState { return MakeKeyState(j.LS, n); }},
    {"rs",   [](const UnitreeJoystick& j, std::string_view n)->KeyState { return MakeKeyState(j.RS, n); }},
    {"lb",   [](const UnitreeJoystick& j, std::string_view n)->KeyState { return MakeKeyState(j.LB, n); }},
    {"rb",   [](const UnitreeJoystick& j, std::string_view n)->KeyState { return MakeKeyState(j.RB, n); }},
    {"a",    [](const UnitreeJoystick& j, std::string_view n)->KeyState { return MakeKeyState(j.A, n); }},
    {"b",    [](const UnitreeJoystick& j, std::string_view n)->KeyState { return MakeKeyState(j.B, n); }},
    {"x",    [](const UnitreeJoystick& j, std::string_view n)->KeyState { return MakeKeyState(j.X, n); }},
    {"y",    [](const UnitreeJoystick& j, std::string_view n)->KeyState { return MakeKeyState(j.Y, n); }},
    {"up",   [](const UnitreeJoystick& j, std::string_view n)->KeyState { return MakeKeyState(j.up, n); }},
    {"down", [](const UnitreeJoystick& j, std::string_view n)->KeyState { return MakeKeyState(j.down, n); }},
    {"left", [](const UnitreeJoystick& j, std::string_view n)->KeyState { return MakeKeyState(j.left, n); }},
    {"right",[](const UnitreeJoystick& j, std::string_view n)->KeyState { return MakeKeyState(j.right, n); }},
    {"f1",   [](const UnitreeJoystick& j, std::string_view n)->KeyState { return MakeKeyState(j.F1, n); }},
    {"f2",   [](const UnitreeJoystick& j, std::string_view n)->KeyState { return MakeKeyState(j.F2, n); }},
    {"lx",   [](const UnitreeJoystick& j, std::string_view n)->KeyState { return MakeKeyState(j.lx, n); }},
    {"ly",   [](const UnitreeJoystick& j, std::string_view n)->KeyState { return MakeKeyState(j.ly, n); }},
    {"rx",   [](const UnitreeJoystick& j, std::string_view n)->KeyState { return MakeKeyState(j.rx, n); }},
    {"ry",   [](const UnitreeJoystick& j, std::string_view n)->KeyState { return MakeKeyState(j.ry, n); }},
    {"lt",   [](const UnitreeJoystick& j, std::string_view n)->KeyState { return MakeKeyState(j.LT, n); }},
    {"rt",   [](const UnitreeJoystick& j, std::string_view n)->KeyState { return MakeKeyState(j.RT, n); }},
  };
