...
private:
    ...
    void run_()
    {
        ...
        static bool printed_keyboard_help = false;
        if (!printed_keyboard_help) {
            spdlog::info("Keyboard fallback: [2] Passive->FixStand, [3] FixStand->Velocity, [1] -> Passive");
            printed_keyboard_help = true;
        }
        if (FSMState::keyboard) {
            std::string key = FSMState::keyboard->key();
            if (FSMState::keyboard->on_pressed && !key.empty()) {
                if (key == "1") nextStateMode = 1;
                else if (key == "2" && currentState->getStateString() == "Passive") nextStateMode = 2;
                else if (key == "3" && currentState->getStateString() == "FixStand") nextStateMode = 3;
            }
        }
